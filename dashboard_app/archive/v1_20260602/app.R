...
ui <- bs4DashPage(
  ...
  body = dashboardBody(
    useShinyjs(),
    tags$head(
      tags$link(rel = "stylesheet", type = "text/css", href = "custom.css?v=40"),
            ...
            column(4,
              ...
              div(style = "background:#f1f5f9;border-radius:12px;padding:20px;margin-top:15px;",
                ...
                p(style = "color:#475569;font-size:0.95rem;margin:0;",
                  ...
                  "Click and drag on the map to draw a circle around a group — a survival curve for those patients will appear below the map. ",
            ...
            column(6,
              ...
              div(style = "background:#f1f5f9;border-radius:12px;padding:20px;margin-top:12px;",
                ...
                p(style = "color:#475569;font-size:0.95rem;margin:0;",
                  ...
                  "The blue line shows the overall trend — if it goes up, the gene is more active in immune-active tumors. ",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$showCalc_km, {
    showModal(modalDialog(title = "How This Was Calculated", size = "l", easyClose = TRUE,
      ...
      h4("Sample Size"),
      p("636 patients — ~318 in each group (high/low)."),
      h4("Caveats"),
      tags$ul(
  ...
  observeEvent(input$showCalc_map, {
    showModal(modalDialog(title = "How This Was Calculated", size = "l", easyClose = TRUE,
      ...
      h4("Caveats"),
      tags$ul(
        tags$li("UMAP is a stochastic algorithm — running it again may produce a slightly different layout."),
  ...
  output$factor_clinical_box <- renderUI({
    ...
    clinical_notes <- list(
      ...
      "Factor2" = list(
        ...
        survival = sprintf("Shows a borderline protective trend. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). Patients with higher Factor 2 scores show a trend toward longer survival, with an RMST difference of %+d days (%s). This pattern is consistent with the known better prognosis of luminal (ER-positive) breast cancers.", hr_all[["Factor2"]], fmt_p(pv_all[["Factor2"]]), ci_l_all[["Factor2"]], ci_u_all[["Factor2"]], rdiff_all[["Factor2"]], fmt_p(rpv_all[["Factor2"]])),
        ...
        bottom = "Captures the well-known favorable prognosis associated with ER-positive, luminal breast cancer making it consistent with decades of clinical knowledge."
      ),
      "Factor3" = list(
      ...
      "Factor4" = list(
        ...
        survival = sprintf("Borderline risk trend. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). The RMST difference is %+d days (%s). The trend suggests younger patients with higher Factor 4 scores may have modestly worse outcomes, but this does not reach statistical significance.", hr_all[["Factor4"]], fmt_p(pv_all[["Factor4"]]), ci_l_all[["Factor4"]], ci_u_all[["Factor4"]], rdiff_all[["Factor4"]], fmt_p(rpv_all[["Factor4"]])),
        ...
        bottom = "Reflects the known biological phenomenon of age-related DNA methylation changes, with a subtle signal that younger patients with this methylation pattern may face slightly higher risk."
      ),
      "Factor5" = list(
        ...
        survival = sprintf("STATISTICALLY SIGNIFICANT protective effect. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). Patients with a 1-unit higher Factor 5 score have %.0f%% lower risk of death at any moment. The effect persists after adjusting for age and stage (adjusted HR = 0.78, p = 0.02). RMST difference: %+d days (%s) which is consistent but underpowered for this metric.", hr_all[["Factor5"]], fmt_p(pv_all[["Factor5"]]), ci_l_all[["Factor5"]], ci_u_all[["Factor5"]], (1 - hr_all[["Factor5"]]) * 100, rdiff_all[["Factor5"]], fmt_p(rpv_all[["Factor5"]])),
      ...
      "Factor7" = list(
        ...
        summary = sprintf("Factor 7 shows a unique survival pattern: it has only a modest Cox effect but is the ONLY factor with statistically significant RMST (restricted mean survival time) benefit. This paradox occurs because Factor 7's protective effect is DELAYED. It appears only after several years of follow-up, which Cox regression (measuring instantaneous risk) misses, but RMST (measuring cumulative survival time) captures.", vtot_all[["Factor7"]]),
        ...
        survival = sprintf("Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f) which is borderline significant. MORE IMPORTANTLY, the RMST difference is %+d days (%s) making it the largest and ONLY statistically significant RMST benefit across all 15 factors. Patients with high Factor 7 live approximately %d days longer within 5 years. This delayed protective effect would be missed by standard Cox analysis alone.", hr_all[["Factor7"]], fmt_p(pv_all[["Factor7"]]), ci_l_all[["Factor7"]], ci_u_all[["Factor7"]], rdiff_all[["Factor7"]], fmt_p(rpv_all[["Factor7"]]), abs(rdiff_all[["Factor7"]])),
      ...
      "Factor8" = list(
        ...
        survival = sprintf("STATISTICALLY SIGNIFICANT risk effect. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). Patients with a 1-unit higher Factor 8 score have %.0f%% higher mortality risk. The effect persists after adjusting for age and stage (adjusted HR = 1.18, p = 0.03). RMST: %+d days (%s).", hr_all[["Factor8"]], fmt_p(pv_all[["Factor8"]]), ci_l_all[["Factor8"]], ci_u_all[["Factor8"]], (hr_all[["Factor8"]] - 1) * 100, rdiff_all[["Factor8"]], fmt_p(rpv_all[["Factor8"]])),
      ...
      "Factor10" = list(
        subtitle = "Risk Factor with Strong Luminal Association",
        summary = sprintf("Factor 10 is a biological paradox. It is a factor with very strong associations with favorable clinical features (ER-positive, Luminal subtype) yet a neutral-to-risk survival profile. Explains %.1f%% of variation and is primarily RNA-driven.", vtot_all[["Factor10"]]),
        ...
        survival = sprintf("Neutral to mild risk. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). RMST difference: %+d days (%s). Despite being strongly linked to favorable Luminal biology (which normally means good prognosis), Factor 10 does NOT show a protective effect, suggesting it captures a specific Luminal B or more aggressive subset within ER-positive disease.", hr_all[["Factor10"]], fmt_p(pv_all[["Factor10"]]), ci_l_all[["Factor10"]], ci_u_all[["Factor10"]], rdiff_all[["Factor10"]], fmt_p(rpv_all[["Factor10"]])),
        omics = sprintf("Nearly pure RNA-driven: RNA = %.1f%%, Methylation = %.1f%%, RPPA = %.1f%%. Top RNA: C19orf20, COL7A1 (collagen), WNT9A (Wnt signaling), SH3D20. The WNT9A and COL7A1 features suggest an extracellular matrix / developmental signaling component.", vrna_all[["Factor10"]], vmeth_all[["Factor10"]], vrppa_all[["Factor10"]]),
        bottom = "The strongest clinical correlations of any factor, yet no survival benefit. It is a reminder that not all strong biological signals are prognostic. May identify Luminal B-like tumors within the broader ER-positive group."
      ),
      "Factor11" = list(
        subtitle = "Mixed Methylation Factor",
        summary = sprintf("Factor 11 is a methylation-driven factor (%.1f%% of signal from DNA methylation) with moderate associations with ER-positive, Luminal biology. Explains %.1f%% of total variation.", vmeth_all[["Factor11"]], vtot_all[["Factor11"]]),
        clinical = "Moderately associated with female sex (r = +0.25, p < 0.001), ER-positive status (r = +0.21, p < 0.001), and Luminal PAM50 subtype (r = +0.23, p < 0.001). No significant age association.",
        survival = sprintf("Mild protective trend (not significant). Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). RMST difference: %+d days (%s). The trend toward protection is consistent with its association with favorable Luminal biology, but the effect is weak.", hr_all[["Factor11"]], fmt_p(pv_all[["Factor11"]]), ci_l_all[["Factor11"]], ci_u_all[["Factor11"]], rdiff_all[["Factor11"]], fmt_p(rpv_all[["Factor11"]])),
        ...
        bottom = "A methylation factor tracking with Luminal biology but with weak prognostic signal. May contribute to RSF model performance despite non-significant univariate Cox result."
      ),
      "Factor12" = list(
      ...
      "Factor13" = list(
        subtitle = "Weak Clinical Signal (Methylation-Driven)",
        summary = sprintf("Factor 13 has the weakest clinical associations of all factors, explaining only %.1f%% of variation. It is largely methylation-driven (%.1f%% of signal) with undefined biological significance.", vtot_all[["Factor13"]], vmeth_all[["Factor13"]]),
        clinical = "Only a weak positive correlation with PAM50 luminal markers (r = +0.09, p = 0.02). No significant associations with age, sex, or ER status. Essentially independent of known clinical variables.",
        survival = sprintf("Non-significant risk trend. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). RMST difference: %+d days (%s). Despite weak univariate signal, LASSO regression (a machine learning feature selection method) identified Factor 13 as the single most important predictor in a multivariable model This suggests that it may capture a complex, non-linear risk pattern missed by standard Cox analysis.", hr_all[["Factor13"]], fmt_p(pv_all[["Factor13"]]), ci_l_all[["Factor13"]], ci_u_all[["Factor13"]], rdiff_all[["Factor13"]], fmt_p(rpv_all[["Factor13"]])),
        ...
        bottom = "A statistical enigma: no clear clinical or biological signal, yet selected as top predictor by LASSO. May represent a complex interaction effect that requires larger sample sizes to fully characterize."
      ),
      "Factor14" = list(
        subtitle = "Top RSF Risk Factor (Methylation-Driven)",
        summary = sprintf("Factor 14 is the most important factor in the Random Survival Forest (RSF) machine learning model (approximately 3 times more important than any other factor). Despite this, it explains only %.1f%% of variance and shows only a modest Cox signal. This suggests Factor 14's effect on survival is non-linear or involves complex interactions that only machine learning methods can detect.", vtot_all[["Factor14"]]),
        ...
        survival = sprintf("Modest but consistent risk signal. Hazard ratio: HR = %.2f (%s, 95%% CI: %.2f\u2013%.2f). RMST difference: %+d days (%s). The RSF importance score (%.4f) is the highest across all factors but the Cox signal alone is not statistically significant. This discrepancy is exactly why multiple analytical methods are used: Cox measures linear, proportional-hazard effects, while RSF captures complex non-linear patterns.", hr_all[["Factor14"]], fmt_p(pv_all[["Factor14"]]), ci_l_all[["Factor14"]], ci_u_all[["Factor14"]], rdiff_all[["Factor14"]], fmt_p(rpv_all[["Factor14"]]), 0.0194),
        ...
        bottom = "THE MYSTERY RISK FACTOR: Top-ranked by machine learning but poorly understood biologically. Its methylation-driven nature and unknown biological function make it a priority for future investigation as it may represent an entirely novel prognostic pathway."
      ),
      "Factor15" = list(
